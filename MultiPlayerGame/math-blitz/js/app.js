/**
 * math-blitz/js/app.js
 * -----------------------------------------------------------------------
 * First player to answer a generated math question correctly scores a
 * point; first to WIN_SCORE wins. Each question has a 10s timer; if
 * nobody answers correctly in time it's skipped with no score change.
 * The whole generated question (text + shuffled options + correct
 * index) is written straight into the room by whichever client starts
 * the round, so both players always see the identical question without
 * needing synchronized randomness (see questions.js).
 * -----------------------------------------------------------------------
 */
...
const shell = new GameShell({ gameTitle: 'Math Blitz Duel' });
...
let botAnswerPendingId = null;
let roundTimerInterval = null;
let roundTimeoutFired = null;

function buildInitialRoom(difficulty) {
  return {
...
async function handlePlay(rawNickname) {
  ...
  shell.setBusy(true);
  try {
    const { roomCode } = await quickMatch.findOrCreateMatch(() => buildInitialRoom(difficulty), generateRoomCode);
...
function attachListeners() {
  ...
  playAgainBtn.addEventListener('click', () => {
    ...
    roomRef.transaction((room) => {
      ...
      room.question = generateQuestion(room.difficulty || 'medium');
...
async function attemptAnswer(seat, optionIndex) {
  const txResult = await roomRef.transaction((room) => {
    ...
    if (room.answers[seat] != null) return; // already answered this round
    room.answers[seat] = optionIndex;

    if (!room.solvedBy && optionIndex === room.question.correctIndex) {
      room.solvedBy = seat;
  ...
  if (txResult.committed) {
    ...
    const wasCorrect = optionIndex === room.question.correctIndex;
...
function maybeTriggerBotAnswer(room) {
  ...
  const key = room.round + '-' + (room.question ? room.question.text : '');
  if (botAnswerPendingId === key) return;
  botAnswerPendingId = key;
  clearTimeout(botAnswerTimeout);

  const willBeCorrect = Math.random() < 0.55;
  const chosen = willBeCorrect
    ? room.question.correctIndex
    : (room.question.correctIndex + 1 + Math.floor(Math.random() * (room.question.options.length - 1))) % room.question.options.length;
...
function renderRoom(room) {
  ...
  if (lastRenderedRound !== room.round) {
